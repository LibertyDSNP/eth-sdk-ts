...
export const snapshotHardhat = async () => {
    ...
    const snapshotResponse: any = await new Promise((resolve, reject) => 
        provider.send({jsonrpc: "2.0", method: "evm_snapshot", id: 1, params: []},
        (err, snapshotResponse) => err ? reject(err) : resolve(snapshotResponse)));
...
export const revertHardhat = async () => {
    ...
    const revertResponse: any = await new Promise((resolve, reject) => 
        provider.send({jsonrpc: "2.0", method: "evm_revert", id: 1, params: [latestSnapshot]}, 
        (err, revertResponse) => err ? reject(err) : resolve(revertResponse)));
