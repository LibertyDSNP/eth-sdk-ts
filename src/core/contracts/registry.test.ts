...
describe("registry", () => {
  ...
  describe("#register", () => {
    ...
    it("returns a Contract Transaction that can be resolved into a DSNP User Id", async () => {
      ...
      expect(await getIdFromRegisterTransaction(transaction)).toEqual("0x03e9"); // 1001
    });
  });
  ...
  describe("getDSNPRegistryUpdateEvents", () => {
    it("can pull one event", async () => {
      ...
      expect(regs[0].dsnpUserId).toEqual("0x0" + Number(1000).toString(16));
